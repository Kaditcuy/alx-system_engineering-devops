...
<p> For this web stack debugging task, we are testing how well our nginx web server setup is doing under pressure and it turns out it is not doing well. We got a huge amount of failed requests.
...
## Fix :
Checked the error log files and this was shown:
`2023/03/09 09:23:03 [crit] 34#0: *1989 open() "/usr/share/nginx/html/index.html" failed (24: Too many open files), client: 127.0.0.1, server: localhost, request: "GET / HTTP/1.0", host: "localhost"
` which shows that the webserver was opening too much files, so i had to find a way to either increase the limit of file descriptors or reduce the number of opened files descriptors. 

In the `/etc/default/nginx` file i noticed ulimit was set to 15, i wrote a puppet script `0-the_sky_is_the_limit_not.pp` that when applied would increase the ulimit to 4096 and the error was solved :)
